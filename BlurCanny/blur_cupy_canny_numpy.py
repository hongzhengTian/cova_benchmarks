...
def convolve2d_cp(image, kernel):
...
def gaussian_blur(image, kernel_size):
    ...
    conved_image = convolve2d_cp(image_cp, kernel)
    image_np = cp.asnumpy(conved_image)
    return image_np

def convolve2d_np(image, kernel):
    kernel = kernel - kernel.mean()
    k_height, k_width = kernel.shape
    h, w = image.shape
    new_image = np.zeros((h - k_height + 1, w - k_width + 1))
    for i in range(new_image.shape[0]):
        for j in range(new_image.shape[1]):
            new_image[i][j] = np.sum(image[i:i+k_height, j:j+k_width] * kernel).astype(np.float32)
    return new_image
...
def sobel_filters(img):
    ...
    Ix = convolve2d_np(img, Kx)
    Iy = convolve2d_np(img, Ky)

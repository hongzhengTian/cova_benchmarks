...
void sobel_filter(image_t &image, image_t_after_conv &gradient_magnitude, image_t_after_conv &gradient_direction) {
    float sum_x, sum_y, angle;
        ...
        for (int j = 1; j < WIDTH_AFTER_CONV - 1; j++) {
            ...
            for (int ki = 0; ki < SOBEL_KERNEL_SIZE; ki++) {
                for (int kj = 0; kj < SOBEL_KERNEL_SIZE; kj++) {
                    int x = i + ki - 1;
                    int y = j + kj - 1;
                    sum_x += image[x][y] * Gx[ki][kj];
                    sum_y += image[x][y] * Gy[ki][kj];
                }
            }

            // Calculate gradient magnitude and direction
            gradient_magnitude[i][j] = hls::sqrt(sum_x * sum_x + sum_y * sum_y);
            angle = hls::atan2(sum_y, sum_x);
...
void canny_edge_top(hls::stream<stream_type> &input_stream, hls::stream<stream_type> &output_stream) {
    ...
    // Perform convolution
    // convolve2d(image, Gx, new_image);
    sobel_filter(image, gradient_magnitude, gradient_direction);
